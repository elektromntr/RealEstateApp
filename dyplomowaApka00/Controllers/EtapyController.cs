...
namespace dyplomowaApka00.Controllers
{
    public class EtapyController : Controller
    {
        ...
        public ActionResult Details(int? id, string niedostepne)
        {
            ...
            var mieszkania = db.Mieszkania.Where(m => m.EtapId == id);
            if (id == null)
            {
            ...
            {
                case "ukryj":
                    mieszkania = mieszkania.Where(m => m.StatusId < 4); // StatusId < 4 powoduje wyświetlanie mieszkań tylko wolnych
                    break;
                default:
                    mieszkania = mieszkania.Where(m => m.StatusId > 0); // StatusId > 0 powoduje wyświetlanie mieszkań wolnych
                    break;
            }
